...
public class PaymentProviderConfig {
     ...
     * @return this config (fluent interface)
     */
    public PaymentProviderConfig addHeader(String key, String value) {
    ...
    /**
     * Adds a payment parameter
     *
     * @param key parameter name
     * @param value parameter value
     * @return this config (fluent interface)
     */
    public PaymentProviderConfig addPaymentParam(String key, Object value) {
        if (this.paymentParams == null) {
            this.paymentParams = new HashMap<>();
        }
        this.paymentParams.put(key, value);
        return this;
    }

    /**
     * Checks if the provider has a percentage fee
     *
     * @return true if percentage fee is set
     */
    public boolean hasPercentageFee() {
        return feePercent != null && feePercent.compareTo(BigDecimal.ZERO) > 0;
    }

    /**
     * Checks if the provider has a fixed fee
     *
     * @return true if fixed fee is set
     */
    public boolean hasFixedFee() {
        return feeFixed != null && feeFixed.compareTo(BigDecimal.ZERO) > 0;
    }

    /**
     * Gets the combined fee as a string for display
     *
     * @return formatted fee string
     */
    public String getFormattedFee() {
        if (hasPercentageFee() && hasFixedFee()) {
            return String.format("%.2f%% + %.2f", feePercent, feeFixed);
        } else if (hasPercentageFee()) {
            return String.format("%.2f%%", feePercent);
        } else if (hasFixedFee()) {
            return String.format("%.2f", feeFixed);
        }
        return "0";
    }
    ...
    public BigDecimal calculateFee(BigDecimal amount) {
        ...
        if (hasPercentageFee()) {
            fee = fee.add(amount.multiply(feePercent).divide(BigDecimal.valueOf(100)));
        }
